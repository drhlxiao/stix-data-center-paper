...
to a NoSQL database. 
The NoSQL database is schemaless,  making it ideal to store data with complex
structures but small sizes like STIX level-1 packets. 
In addition to packets, other data extracted during data parsing are store in the NoSQL database,
 for example,  
 
\begin{itemize}
  \item Raw file metada, 
  \item Bulk science metada;
  \item Quick-look dataset;
  \item STIX instrument configuration database;
\end{itemize}
The database for pipelines or APIs from packets, and stores them into several datasets in the NoSQL database, for example,


such as data arrival 
times, data acquisition time ranges,  calibration run configuration parameters,  raw data 
filenames. They are written to several datasets in the NoSQL database. 
The metadata allows for query and fast quick-look of necessary for the subsequent processing. 
...
\end{figure*}
Estimation of the coarse location of a flare on ground is started after receiving the level-1 pixel data. 
...
\begin{itemize}
  ...
  \item Background subtraction. Using the 
\end{itemize}
