...
a new ELUT is created and commanded to STIX after being validated by the operations team. 
...
%Solar flare identification on the ground is started 
%after each new QL light curve dataset is pushed to the NoSQL database. 
%As  \cite{andrea2021}
...
%we studied correlation between peak flux measured by GOE16  and STIX  count rate.
...
The wide spread can be explained by the difference in 
the energy response of the two instruments and in the flare temperature.
...
Parameterized Eq.~\ref{eq:goes-stix} is used to estimate GOES classes for flares 
which are not in the earth FOV. The results are stored in the flare list in the database, which is accessible 
