...
\subsubsection{Solar flare image reconstruction pipeline}
...
To facilitate selection of flares of interest, a flare image reconstruct pipeline 
has been developed and integrated into the main data processing pipeline. 
The pipeline uses  the flare list and the science data metadata recorded in the NoSQL database 
to find available signal data and data for suitable background data for each flare. 
For each flare, pixel counts recorded within one-minute time frame around the flare peaks  
are selected for background subtraction, transmission and dead time corrections. 
The corrected counts are integrated over the energy ranges 
of 4 -- 10 keV (thermal emission) and 16 -- 28 (non-thermal)  keV for calculations of visibilities. 
Then images are reconstructed from the visibilities using five different algorithms: Back-projection, CLEAN, 
EM and VIS\_FWDFIT \cite{paolo2020}.
As an example, Fig.~\ref{fig:imaging} shows the images reconstructed for the flare observed at 2022-03-08T08:55:17Z. 
The light curves and time range selected for imaging are shown in the first panel of the figure. 

Reconstructed images as well as the 
ephemeris data are written to FITS files. 
At the mean time, the metada of images are recorded in the database.  
They are further used for creation of images in the PNG and PDF formats, which
are accessible with web applications or via python APIs (see \ref{sec:apis}). 


\subsubsection{Spectral analysis}

Energy spectra provide direct information on electron 
acceleration in solar flares. 
STIX team uses 
the OSPEX2 SSWIDL package  to fit solar hard X-ray spectra. 
The software includes a wide
...
and calls  OSPEX routines for spectral fitting. A python implementation of the fitting package is  
being developed by STIX team and will be integrated 
into the main data processing pipeline, which will provide spectroscopic data products for the community. 

\section{Science data request automation}
...
They are down-link to the ground only after receiving data request telecommands initiated  by the STIX team. 
A data request telecommand provides information on the
time range, time binning, energy range, energy binning  and selected detectors (or pixels), based on 
...
After two years of operation, a semi-automated data selection procedure has been established,
which involves the following steps:
\begin{itemize}
  \item  
    High time and energy resolution pixel data (onboard level-1) is requested for each solar flare 
    with the peak signal count rate above a threshold (typically 125 counts/seccond).  
    The energy range is  limited to the range that has signal counts, and the time resolutions is 
    adjusted to comply with the constraint on telemetry data volume.  
    The higher the allocated data volume, the higher time resolution is requested.
  \item If the count rate of a flare is below the count rate limit mentioned above,
   and  the total number recorded signal counts is less than a threshold, 
  only single time-bin pixel data are requested.  The threshold on the total signal counts
   is chosen at a value so that the statistics are sufficient to reconstruct an image.
 \item  Spectrograms require small amount data, therefore high time and energy resolution spectrograms
  are requested for all time periods when STIX is in NOMINAL mode. 
  The maximum duration of a single spectrogram request is 6 hours and the highest possible time resolution and energy resolution 
  are requested. 
...
\section{STIX data interfaces and data analysis tools}
\subsection{Web-based data managers and browsers}
STIX data center pipelines generates various data products and metadata. 
