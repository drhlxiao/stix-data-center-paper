...
has been developed and integrated with the main data processing pipeline. 
The pipeline  uses  the flare list and the metadata of level-1 data in the NoSQL database 
to find signal data and data for suitable background data for each flare. 
...
Then images are reconstructed from the visibilities with Back-projection, CLEAN,  EM, and VIS\_FWDFIT \cite{paolo2020} algorithms.
Fig.~\ref{fig:imaging} shows an example of  images reconstructed for a flare observed at 2022-03-08T08:55:17Z. 
The light curves and selected time range for imaging are shown in the first panel of the figure. 
...
it reads pixel count data from L1 FITS files, 
corrections for dead time, transmission and energy binning for the counts,  and calls 
OSPEX routines for spectral fitting.  
In the future, level-3 fits files, in which detector effects are corrected, will be used. 
...
before they are overwritten, are only down-linked after being requested from the ground. 
Selection  of science data on-ground  is based on the knowledge from quick-look light curves 
and is a tedious task, as it must take
into account many factors, 
such as constraints on the  allocated telemetry data rate, 
the daily maximum number of requests that can be submitted, 
age of data, solar flare fluxes and scientific value of data. 
After two years of continuous optimization, the data selection has been semi-automated,
which involves the following steps:
\begin{itemize}
  \item  
  For each flare, if the recorded photons for image reconstruction (about 1000 photons), 
