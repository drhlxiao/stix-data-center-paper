...
\begin{figure*}
  ...
  \caption{Left: simulated shadow pattern (black color) of CFL sub-collimator on STIX detector.
    Left: area of exposed regions of 12 pixels calculated by combining
...
\begin{itemize}
  ...
  \item Background subtraction. The most recent level-1 dataset for the quiet sun period is used for background subtraction;
  \item Calculation of the mean fluence except for the CFL and the background detectors; 
      ...
      Therefore, the fluence of a detector can be given by $c/(r*S_d)$,  where $c$ is the registered 
      count and $S_\textrm{det}$ the total area of 12 pixels. 
  \item Calculation of illuminated area of CFL pixel $s_i$ 
  %%\item 
\end{itemize}
