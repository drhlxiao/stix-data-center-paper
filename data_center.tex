...
\subsubsection{Image reconstruction pipeline}
...
\begin{figure*}
  ...
  \label{fig:imaging}
\end{figure*}
STIX observes thousands of flares every year.
In order to better select events of interest,  
an automated image reconstruction 
pipeline has been developed and integrated with the STIX data center. 

The pipeline selects flares with peak 
counts above a certain limit and constructs images for
 a thermal and a non-thermal energy  
ranges,  using Back-projection, CLEAN,  EM, and VIS\_FWDFIT \cite{paolo2020} for peak time periods. 
Fig.~\ref{fig:imaging} shows an example of time range selected, and images reconstructed by the pipeline. 
Reconstructed images are written to FITS files, PNG, and PDF files. 
They are accessible at the STIX data center. 
 
\section{Data request and automation}

High time and energy resolution pixel data stored
in the onboard archive memory contain much richer information than low latency data.
Due to the data rate constraints,  they are not down-linked automatically and 
need to be selected on the ground for downloading.
STIX science data are selected on ground automatically.
Selections of science data involve optimizations of time intervals, 
