...
It should be noted that more observations will be included into the fit.
...
are accessible with web applications or via python application interfaces (APIs, see \ref{sec:apis}). 
...
\begin{itemize}
 ...
 \item Spectrograms (onboard level-4) require relative small amount data, therefore
  the highest possible time resolution and energy resolution are requested for all time periods that STIX is in NOMINAL mode. 
...
After being reviewed and adjusted by STIX team, data requests 
are selected from the database and compiled to instrument operation requests (IORs). 
They are used to create the final telecommands by ESA.
...
Data products generated by pipelines are either stored directly in the NoSQL database, 
or in individual files.  For the latter, only metadata are stored in the database.
They can be accessed via web interfaces or python application interfaces. 
