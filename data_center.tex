...
As we mentioned earlier, due to bandwidth constraints, stix will only automatically download low-latency data. 
Quick-look data are energy-integrated data of low temporal resolution; 
these data do not provide the spectroscopy we are interested in, as well as flare image data. 
The pixel data stored in the stix memory contains much richer information. To download these data, it is necessary to select the data on the ground, determine the time period of the data of interest, the energy segment, the energy merge, and the time merge information. According to the limitation of the data capacity, these parameters are sent by sending remote control commands to, 
and the download it after the host computer processes it.
...
The data request is stored in the database. According to the priority of the data request,
 the generation time of the data, and the bandwidth limit, each data request is assigned a 10-digit number,
...
Web-based applications are selected for the graphical
 interfaces at the data center. This process enables the advantages 
 ...
 The client side and server side interact and operate as shown in Fig. 5. The details are given below:
