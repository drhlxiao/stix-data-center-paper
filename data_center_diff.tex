...
%DIF DEL data_center.tex      Wed Feb  1 16:04:52 2023
%DIF ADD data_center_v2.tex   Wed Mar 15 10:54:52 2023
%DIF < \documentclass[referee]{preaa} % for a referee version
%DIF -------
...
\begin{document} 
   ...
   \author{Hualin Xiao
          ...
          \and Ewan Dickson \DIFdelbegin %DIFDELCMD < \inst{4}
%DIFDELCMD <           %%%
\DIFdelend \DIFaddbegin \inst{3}
          \DIFaddend \and Paolo Massa \DIFdelbegin %DIFDELCMD < \inst{6}
%DIFDELCMD <           %%%
\DIFdelend \DIFaddbegin \inst{4}
          \DIFaddend \and Erica Lastufka \inst{1}
          \and Andrea Francesco Battaglia\DIFdelbegin %DIFDELCMD < \inst{1,3}
%DIFDELCMD <           %%%
\DIFdelend \DIFaddbegin \inst{1,5}
          \DIFaddend \and László Etesi \inst{1}
          \and Nicky Hochmuth \DIFdelbegin %DIFDELCMD < \inst{8}
%DIFDELCMD <           %%%
\DIFdelend \DIFaddbegin \inst{6}
          \DIFaddend \and Fr\'ed\'eric Schuller \inst{7}
          \and Daniel F.\ Ryan \inst{1}
          \and Olivier Limousin \DIFdelbegin %DIFDELCMD < \inst{5}
%DIFDELCMD <           %%%
\DIFdelend \DIFaddbegin \inst{8}
          \DIFaddend \and Hannah Collier \DIFdelbegin %DIFDELCMD < \inst{1,3}
%DIFDELCMD <           %%%
\DIFdelend \DIFaddbegin \inst{1,5}
          \DIFaddend \and Alexander Warmuth \inst{7}
   ...
   \institute{University of Applied Sciences and Arts Northwestern Switzerland (FHNW), 5200 Windisch, Switzerland \\
         ...
         \and \DIFdelbegin \DIFdel{ETH Z\"urich, R\"amistrasse 101, 8092 Z\"urich, Switzerland
         }%DIFDELCMD < \and %%%
\DIFdelend University of Graz, Universitätspl. 3, 8010 Graz, Austria
      \and \DIFdelbegin \DIFdel{Université Paris-Saclay, Université Paris Cité, CEA, CNRS, AIM, 91191 Gif-sur-Yvette, France
      }%DIFDELCMD < \and %%%
\DIFdelend Department of Physics \& Astronomy, Western Kentucky University, Bowling Green, KY 42101, USA
          \and \DIFaddbegin \DIFadd{ETH Z\"urich, R\"amistrasse 101, 8092 Z\"urich, Switzerland
         }\and \DIFadd{Ateleris, Badenerstrasse 13, CH-5200 Brugg Switzerland
         }\and \DIFaddend Leibniz-Institut für Astrophysik Potsdam (AIP), An der Sternwarte 16, D-14482 Potsdam, Germany
         \and \DIFdelbegin \DIFdel{Ateleris}\DIFdelend \DIFaddbegin \DIFadd{Université Paris-Saclay}\DIFaddend , \DIFdelbegin \DIFdel{Badenerstrasse 13}\DIFdelend \DIFaddbegin \DIFadd{Université Paris Cité}\DIFaddend , \DIFdelbegin \DIFdel{CH-5200 Brugg Switzerland
         }\DIFdelend \DIFaddbegin \DIFadd{CEA, CNRS, AIM, 91191 Gif-sur-Yvette, France
         }\DIFaddend \and 
...
Solar Orbiter is \DIFdelbegin \DIFdel{a Sun-observing mission led by ESA with contributions from NASA that 
addresses }\DIFdelend \DIFaddbegin \DIFadd{an ESA-led mission with a suite of ten remote-sensing and in-situ instruments to study }\DIFaddend the interaction between the Sun and the heliosphere. \DIFdelbegin \DIFdel{It was launched }\DIFdelend \DIFaddbegin \DIFadd{Launched }\DIFaddend on 10 February 2020 \DIFdelbegin \DIFdel{, with a nominal mission duration of seven years and }\DIFdelend \DIFaddbegin \DIFadd{into a heliocentric orbit, its nominal mission will last seven years with }\DIFaddend a planned extension of \DIFdelbegin \DIFdel{three }\DIFdelend \DIFaddbegin \DIFadd{a three additional }\DIFaddend years \citep{Mueller2020}.
\DIFdelbegin \DIFdel{It carries ten instruments for comprehensive
remote sensing and in-situ observations.
}\DIFdelend Solar Orbiter's closest approach to the Sun is 0.28\DIFdelbegin \DIFdel{AU and as the mission progresses, }\DIFdelend \DIFaddbegin \DIFadd{\,AU and }\DIFaddend its orbit will shift out of the ecliptic \DIFaddbegin \DIFadd{as the mission progresses}\DIFaddend , providing better views of the solar poles \citep{SolarOrbiter2020}.
\DIFaddbegin 

\DIFaddend Solar Orbiter's Spectrometer Telescope for Imaging X-rays \citep[STIX;][]{stix2020} observes X-rays from 4 to 150 keV.  Its main scientific objective is to study the extremely hot solar plasma and accelerated electrons produced during solar flares.
STIX consists of 32 sub-collimators, each with a pair of slightly offset grids that encode spatial information in a Moiré pattern which is detected by a pixelated cadmium telluride (CdTe) detector. \DIFdelbegin \DIFdel{Using this method, images }\DIFdelend \DIFaddbegin \DIFadd{Images }\DIFaddend with a few arcsec angular resolution can be produced on the ground \DIFdelbegin \DIFdel{using }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend indirect Fourier-based imaging techniques.  STIX's \DIFdelbegin \DIFdel{spectroscopic detectors enable these }\DIFdelend \DIFaddbegin \DIFadd{detectors are spectroscopic and so enable the thermal plasma and accelerated electrons to be characterised via the spectra they produce.
They also enable }\DIFaddend images to be produced in any energy range between 4 to 150 keV \DIFdelbegin \DIFdel{as well as simultaneously providing spectra of the emission}\DIFdelend \DIFaddbegin \DIFadd{and over any integration (exposure) time greater than $\sim$0.5\,s.
This allows scientists to customise the imaging based on their science goals or observational constraints, e.g.\ periods of low count rates}\DIFaddend . Hence, STIX provides observations of the intensity, spectrum, dynamics, and location of accelerated electrons and heated plasma during solar flares. For more information on STIX instrumentation and its scientific capabilities, see \citet{stix2020}.




\DIFdelbegin \DIFdel{STIX observes continuously during normal operations. The data are compressed and binned in time and energy to save onboard storage.  Low-latency telemetry data are sent to Earth during  passes of the ground station (typically one pass a day) and }\DIFdelend \DIFaddbegin \DIFadd{During normal operations, STIX observes and acquires data continuously. Housekeeping (HK) and quicklooks (QL) are sent automatically at regular intervals, whereas other science products }\DIFaddend are \DIFdelbegin \DIFdel{processed automatically by the STIX Data Center into housekeeping and quicklook data products .  These are crucial for monitoring the health and performance of the instrument as well as requesting higher telemetry science-quality data. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The STIX Data Center runs the quicklook data through a flare detection algorithm, which identifies periods of scientific interest for which science-quality data should be requested from the instrument. To save telemetry these data requests can be made at coarser time, energy, and pixel binning than is used natively onboard .  The severity of the binning is chosen depending on the apparent scientific importance of the observations.  Human operators can manually review and edit the data requests compiled by the STIX Data Center and can include requests for additional periods of interest.  Despite this human involvement, the automatic pipelines of the STIX Data Center carry a large fraction of the work and enable STIX operations to be conducted by only a small number of people.
...
%DIFDELCMD < %%%
\DIFdel{Once science data are received, the STIX Data Center automatically processes them into higher-level data products. To help users identify and understand the data products,  }\DIFdelend \DIFaddbegin \DIFadd{requested from the ground. STIX compresses and structures the data into different types of telemetry packets onboard to reduce the amount of data needed to be transmitted. 
The total number of raw telemetry data types reaches hundreds.
To assist with instrument operations, simplify STIX data analysis, increase data accessibility for the solar physics community, and enable regular data deliveries to ESA, automated data processing pipelines and a data platform have been developed at }\DIFaddend the STIX Data Center\DIFdelbegin \DIFdel{performs common analyses for flaring data, such as  the determination of coarse flare locations, imaging, and spectroscopy.
This is particularly helpful for STIX as its indirect imaging technique is not intuitive, and hence its data can be confusing to new users. The STIX }\DIFdelend \DIFaddbegin \DIFadd{.
The pipelines convert raw telemetry data into quick-looks and }\DIFaddend data \DIFdelbegin \DIFdel{center provides various }\DIFdelend \DIFaddbegin \DIFadd{products at different processing levels that can be used for scientific analysis. The platform stores all STIX data products and provides }\DIFaddend web-based \DIFdelbegin \DIFdel{interfaces for viewing data products, metadata created by data processing pipelines,   and results from common analyses}\DIFdelend \DIFaddbegin \DIFadd{tools for users to explore and analyze STIX data}\DIFaddend .


 %DIF < The number of telemetry packets for larger bulk science data requests can reach hundreds of packets.
%DIF < To assist with the complexity of STIX data analysis, make the data more accessible to the solar physics community, and allow for regular data deliveries to ESA, automated data processing pipelines and a data platform have been developed at FHNW. The pipelines convert raw telemetry data into QLs and data products at different processing levels that can be used for scientific analysis. The platform stores all STIX data products and provides web-based tools for users to explore and analyze STIX data.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD <  %%%
\DIFdelend In this paper, we describe the processing pipelines, core analysis algorithms, data products, and other tools used and provided by the STIX Data Center.  The aim is to help users better understand the Data Center and hence how to best utilize it and the STIX instrument.
...
\section{Data \DIFdelbegin \DIFdel{reception}\DIFdelend \DIFaddbegin \DIFadd{flow}\DIFaddend }
\DIFaddbegin \begin{figure*}
    \centering
    \includegraphics[width=0.9\linewidth]{figures/data_flow.pdf}
    \caption{ \DIFaddFL{STIX Data Center hardware architecture and  data flow. Solar Orbiter telemetry data are processed at the Mission Operations Center (MOC) and delivered to the instrument team via the Generic File Transfer System (GFTS). 
    ...
    \label{fig:data-flow}
\end{figure*}
\DIFaddend During nominal science operations, low-latency data are transmitted to Earth during every ground station pass regardless of orbital geometry, whereas science data are only downlinked when bandwidth permits. Every instrument team is responsible for keeping the transmitted data volume within pre-allocated boundaries. 
\DIFdelbegin \DIFdel{ESA's Ground Operation System (EGOS) processes telemetry }\DIFdelend \DIFaddbegin 


\DIFadd{Telemetry }\DIFaddend data received by ground stations \DIFaddbegin \DIFadd{are processed by the Mission Control System, which is a part of the ground system segment, }\DIFaddend at the Solar Orbiter \DIFdelbegin \DIFdel{mission control center. Then the telemetry data }\DIFdelend \DIFaddbegin \DIFadd{Mission Operations Center (MOC). Telemetry data after undergoing quality checks, packet extraction, and filling are stored in the  EGOS Data Dissemination System (EDDS) \mbox{%DIFAUXCMD
\citep{egos, EDDS}}\hspace{0pt}%DIFAUXCMD
. They }\DIFaddend are distributed to the instrument teams regularly via the \DIFdelbegin \DIFdel{EGOS Data Dissemination System (EDDS)\mbox{%DIFAUXCMD
\citep{EDDS}}\hspace{0pt}%DIFAUXCMD
.  
}\DIFdelend \DIFaddbegin \DIFadd{Generic File Transfer system (GFTS).  
...
\DIFaddend Low-latency telemetry data generally arrive at the STIX \DIFdelbegin \DIFdel{data center }\DIFdelend \DIFaddbegin \DIFadd{Data Center }\DIFaddend within a few days, depending on the next ground station pass. However, science data may be delayed 
several weeks after being generated onboard due to possibly low bandwidth or priority.
In addition to telemetry data, the STIX \DIFdelbegin \DIFdel{data center }\DIFdelend \DIFaddbegin \DIFadd{Data Center }\DIFaddend receives auxiliary data \citep{spice1996,spice2018} from the Science Operations Center (SOC), which contain information on spacecraft ephemeris, attitude, and calibration factors required for the conversion of onboard timestamps to UTC times. 
...
\begin{figure*}
    ...
    \caption{Telemetry processing pipelines at STIX \DIFaddbeginFL \DIFaddFL{Data Center. Raw packets are processed to
level-1 which includes parsing, integer decompressing, and timestamp conversion. L1 packets are written to a NoSQL database. Then they are selected and processed in four different paths, each of which is responsible for processing specific }\DIFaddendFL data \DIFdelbeginFL \DIFdelFL{center}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{types}\DIFaddendFL .    }
    \label{fig:main_pipelines}
\end{figure*}
STIX telemetry data are processed immediately upon reception at the STIX \DIFdelbegin \DIFdel{data center }\DIFdelend \DIFaddbegin \DIFadd{Data Center }\DIFaddend by the data processing pipeline  
shown in Fig.~\ref{fig:main_pipelines}.  Raw packets are processed to
level-1 \DIFaddbegin \DIFadd{(L1) }\DIFaddend which includes parsing, integer decompressing, and timestamp conversion. 
...
In the first, \DIFdelbegin \DIFdel{quicklook packets are selected for the identification of solar flares}\DIFdelend \DIFaddbegin \DIFadd{the housekeeping, quicklook, and science data are successively selected and used to create level-1 FITS files}\DIFaddend . 
The second path \DIFdelbegin \DIFdel{performs common analyses of science data for flares.
The third path }\DIFdelend selects calibration data for the determination of energy calibration factors.   
In the \DIFdelbegin \DIFdel{fourth, the housekeeping, quicklook , and science data are successively selected and used to create level-1 FITS files}\DIFdelend \DIFaddbegin \DIFadd{third path, quicklook packets are selected for the identification of solar flares. 
The fourth path performs common analyses of science data for flares}\DIFaddend .
...
\begin{figure*}
  ...
  \caption{
   ...
   Middle: Best-fit flare centroid location (marked by x) and its 1$\sigma$, 2$\sigma$, and 3$\sigma$ confidence contours. \DIFaddbeginFL {\DIFaddendFL Right: Projection of CFL sub-collimator (the gray shaded regions) on the detector \DIFaddbeginFL \DIFaddFL{pixels (the green closed areas) }\DIFaddendFL simulated for the best-fit flare location.  \DIFaddbeginFL \DIFaddFL{The top-row,  bottom-row, and small pixels (from left to right) are numbered sequentially from  0 to 11.  }}\DIFaddendFL }
  \label{fig:cfl}
\end{figure*}
